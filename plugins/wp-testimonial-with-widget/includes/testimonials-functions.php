...
function register_post_type_testimonials () {
		...
		$archive_slug = apply_filters( 'sp_testimonials_archive_slug', _x( 'testimonials', 'post archive url slug', 'wp-testimonial-with-widget' ) );
...
function register_custom_columns ( $column_name ) {
	...
	function get_image ( $id, $size, $style = "circle" ) {
		$response = '';
